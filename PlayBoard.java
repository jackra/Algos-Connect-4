...
public class PlayBoard{
	...
	public void clear() {
		...
		for (int j = 0 ; j < 7; j++) col[j] = 0;        
	}
	public int next() {return next_player;}
	public int[] ret_col(){return col;}
	public boolean over() {
		String line_x="";
		String line_y="";
		try{
			...
			while ( (tempx>0) && (tempy>0)){
				tempx--;tempy--;
				line_ld = (new Integer(location[tempx][tempy])).toString() + line_ld;
			}
			tempx=move_x;tempy=move_y;
			while ( (tempx <5)&& (tempy <6)) {
				tempx++;tempy++;
				line_ld = line_ld+(new Integer(location[tempx][tempy])).toString();
			}
			tempx=move_x;tempy=move_y;
			while ( (tempx>0) && (tempy<6)){
				tempx--;tempy++;
				line_rd = (new Integer(location[tempx][tempy])).toString() + line_rd;
			}
			tempx=move_x;tempy=move_y;
			while ( (tempx <5)&& (tempy >0)) {
				tempx++;tempy--;
				line_rd = line_rd+(new Integer(location[tempx][tempy])).toString();
			}
